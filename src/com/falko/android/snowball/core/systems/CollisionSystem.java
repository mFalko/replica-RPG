...
//        mWorld = world;
    ...
    public boolean castRay(Vector2D startPoint, Vector2D endPoint, Vector2D movementDirection,
            Vector2D hitPoint, Vector2D hitNormal, GameObject excludeObject) {
        ...
        if (mTemporarySegments.getCount() > 0) {
            ...
            Vector2D tempHitNormal = vectorPool.allocate();
    ...
    public boolean testBox(float left, float right, float top, float bottom, 
            Vector2D movementDirection, FixedSizeArray<HitPoint> hitPoints,
        ...
        if (!testDynamicSurfacesOnly) {
            ...
            final int startTileX = Utils.clamp((int)(startX / mTileWidth), 0, mMapWidth - 1);
            final int endTileX = Utils.clamp((int)(endX / mTileWidth), 0, mMapWidth - 1);
            final int startTileY = Utils.clamp((int)(startY / mTileHeight), 0, mMapHeight - 1);
            final int endTileY = Utils.clamp((int)(endY / mTileHeight), 0, mMapHeight - 1);
            
            VectorPool vectorPool = sSystemRegistry.vectorPool;
            Vector2D worldTileOffset = vectorPool.allocate();
            
//            final int[][] tileArray = mWorld.getTiles();
            final int worldHeight = mMapHeight - 1;
...
//            }
        ...
        boolean tempHit = testBoxAgainstList(mTemporarySegments,
                ...
                movementDirection, excludeObject, Vector2D.ZERO, hitPoints);
     ...
     */
    protected int executeStraigtRay(final Vector2D startPoint, final Vector2D endPoint, 
            final int startTileX, final int startTileY, final int endTileX, final int endTileY,
            final int deltaX, final int deltaY,
            Vector2D hitPoint, Vector2D hitNormal, TileVisitor visitor) {
...
//        }
     ...
     * @return  The index of the tile that intersected the ray, or -1 if no intersection was found.
     */
    protected int executeRay(Vector2D startPoint, Vector2D endPoint, 
            Vector2D hitPoint, Vector2D hitNormal, TileVisitor visitor) {
...
//        }
    ...
    protected static boolean testBoxAgainstList(FixedSizeArray<LineSegment> segments, 
        ...
        for (int x = 0; x < count && hitCount < maxSegments; x++) {
            ...
            if (dot < 0.0f &&
                ...
                Vector2D hitNormal = vectorPool.allocate(segment.mNormal);
    ...
    public abstract class TileVisitor extends AllocationGuard {
        ...
        public abstract boolean visit(CollisionTile tile, Vector2D startPoint, Vector2D endPoint,
            Vector2D hitPoint, Vector2D hitNormal, int tileX, int tileY);
    }
    ...
    protected class TileTestVisitor extends TileVisitor {
        ...
        // runtime allocation.
        private Vector2D mDelta;
        private Vector2D mTileSpaceStart;
        private Vector2D mTileSpaceEnd;
        private Vector2D mTileSpaceOffset;
        private int mTileHeight;
        private int mTileWidth;
        ...
        public TileTestVisitor() {
            super();
            mDelta = new Vector2D();
            mTileSpaceStart = new Vector2D();
            mTileSpaceEnd = new Vector2D();
            mTileSpaceOffset = new Vector2D();
        }
         ...
         */
        public void setup(Vector2D movementDirection, int tileWidth, int tileHeight) {
            if (movementDirection != null) {
         ...
         */
        @Override
        public boolean visit(CollisionTile tile, Vector2D startPoint, Vector2D endPoint,
                Vector2D hitPoint, Vector2D hitNormal, int tileX, int tileY) {
    ...
    protected class LineSegment extends AllocationGuard {
        ...
        public LineSegment() {
            ...
            mNormal = new Vector2D();
        }
        
        /* Sets up the line segment.  Values are copied to local storage. */
        public void set(Vector2D start, Vector2D end, Vector2D norm) {
    ...
    /**
     * A pool of collision tiles.
     */
    protected class CollisionTilePool extends TObjectPool<CollisionTile> {
        public CollisionTilePool() {
            super();
        }
        
        public CollisionTilePool(int count) {
            super(count);
        }
        ...
        protected void fill() {
            for (int x = 0; x < getSize(); x++) {
                getAvailable().add(new CollisionTile());
            }
        }

        @Override
        public void release(Object entry) {
            ((CollisionTile)entry).reset();
            super.release(entry);
        }
