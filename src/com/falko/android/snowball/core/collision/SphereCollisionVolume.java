...
public class SphereCollisionVolume extends CollisionVolume {
    ...
    private Vector2D mWorkspaceVector;
    private Vector2D mWorkspaceVector2D;
    
    public SphereCollisionVolume(float radius, float centerX, float centerY) {
        super();
        mRadius = radius;
        mCenter = new Vector2D(centerX, centerY);
        mWorkspaceVector = new Vector2D();
        mWorkspaceVector2D = new Vector2D();
    }
    ...
    @Override
    public boolean intersects(Vector2D position, FlipInfo flip, CollisionVolume other, 
        ...
        if (other instanceof AABoxCollisionVolume) {
            ...
            if (other instanceof SphereCollisionVolume) {
                ...
                mWorkspaceVector2D.set(otherPosition);
                offsetByCenter(mWorkspaceVector2D, sphereOther.getCenter(), otherFlip);
                mWorkspaceVector.subtract(mWorkspaceVector2D);
                ...
                mWorkspaceVector2D.set(otherPosition);
                mWorkspaceVector2D.x += centerX;
                mWorkspaceVector2D.y += centerY;
    ...
    private static void offsetByCenter(Vector2D position, Vector2D center, FlipInfo flip) {
