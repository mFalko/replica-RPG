...
public class HudSystem extends BaseObject {
    ...
    private Vector2D mFlyButtonLocation;
    private boolean mFlyButtonActive;
    private boolean mFlyButtonPressed;
    
    private Vector2D mStompButtonLocation;
    private boolean mStompButtonPressed;
    
    private Vector2D mMovementSliderBaseLocation;
    private Vector2D mMovementSliderButtonLocation;
    ...
    private Vector2D mCoinLocation;
    private Vector2D mRubyLocation;
    ...
    public HudSystem() {
        super();
        mFlyButtonLocation = new Vector2D();
        mStompButtonLocation = new Vector2D();
        mCoinLocation = new Vector2D();
        mRubyLocation = new Vector2D();
        mFPSLocation = new Vector2D();
        ...
        mMovementSliderBaseLocation = new Vector2D();
        mMovementSliderButtonLocation = new Vector2D();
    ...
    public void update(float timeDelta, BaseObject parent) {
        ...
        if (manager != null && manager.getPlayer() != null) {
	        ...
	        if (mFuelDrawable != null && mFuelBackgroundDrawable != null 
	            ...
	            Vector2D location = pool.allocate();
        ...
        if (mFading && factory != null) {
            ...
            if (percentComplete < 1.0f || !mFadeIn) {
                ...
                if (bitmap != null) {
                    ...
                    render.scheduleForDraw(bitmap, Vector2D.ZERO, SortConstants.FADE, false);
                }
            }
...
//            }
    ...
    private void drawNumber(Vector2D location, int[] digits, boolean drawX) {
        final RenderSystem render = sSystemRegistry.renderSystem;
