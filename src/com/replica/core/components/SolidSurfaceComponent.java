...
public class SolidSurfaceComponent extends GameComponent {
    private FixedSizeArray<Vector2D> mStartPoints;
    private FixedSizeArray<Vector2D> mEndPoints;
    private FixedSizeArray<Vector2D> mNormals;
    private Vector2D mStart;
    private Vector2D mEnd;
    private Vector2D mNormal;
    ...
    public void inititalize(int maxSurfaceCount) {
        if (mStartPoints == null 
            ...
            mStartPoints = new FixedSizeArray<Vector2D>(maxSurfaceCount);
            mEndPoints = new FixedSizeArray<Vector2D>(maxSurfaceCount);
            mNormals = new FixedSizeArray<Vector2D>(maxSurfaceCount);
        }
    ...
    public void addSurface(Vector2D startPoint, Vector2D endPoint, Vector2D normal) {
    ...
    public void update(float timeDelta, BaseObject parent) {
        ...
        final FixedSizeArray<Vector2D> startPoints = mStartPoints;
        final FixedSizeArray<Vector2D> endPoints = mEndPoints;
        final FixedSizeArray<Vector2D> normals = mNormals;
        
        final int surfaceCount = startPoints.getCount();
        if (collision != null && surfaceCount > 0) {
            GameObject parentObject = (GameObject)parent;
            final Vector2D position = parentObject.getPosition();
            Vector2D start = mStart;
            Vector2D end = mEnd;
            Vector2D normal = mNormal;
